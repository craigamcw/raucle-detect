...
``raucle-detect audit verify``). The gate never reads the agent's reasoning, so
a prompt-injected call is denied on the same path as a legitimate one.

Install with ``pip install 'raucle-detect[crewai]'``.
"""
...
def guard_tool(
    ...
    *,
    ...
    if not _HAS_CREWAI:
        raise RuntimeError(
            "guard_tool requires crewai. Install with: pip install 'raucle-detect[crewai]'"
        )
