...
The proof proceeds by case analysis on $k$. For each constraint kind, `attenuate`'s implementation is shown to compute the appropriate lattice meet (intersection, union, min, max). Because each operation is monotone in the lattice, $\sqsubseteq_k$ is preserved by definition. The lifetime bound follows from the explicit guard in `attenuate` that raises on $c.\text{expires\_at} > p.\text{expires\_at}$. The full Lean development is roughly `[LEAN ~150 lines]`.
...
The proof is a direct case analysis on the eight gate checks. Each check, if it returned ALLOW, established the corresponding postcondition. The pre-image analysis is mechanical. `[LEAN ~200 lines]`.
...
The proof composes the prover's soundness — every assignment satisfying $S$ that violates $P$ is found by Z3, so `PROVEN` certifies $\Sigma \cap \neg P = \emptyset$ — with Theorem 2 and the constraint-lattice tightness assumption on $t$. `[LEAN ~250 lines, depends on a small axiomatic interface to the Z3 oracle]`.
