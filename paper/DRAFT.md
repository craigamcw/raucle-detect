...
**Theorem 3 (Policy-Proof Composition).** *Let $S$ be a schema, $P$ a policy, $\rho$ a proof artifact from $\text{Prove}(S, P)$ with $\rho.\text{status} = \text{PROVEN}$, and $t$ a token whose $\text{policy\_proof\_hash} = \rho.\text{hash}$ and whose constraints are pointwise at least as tight as $P$ under the constraint lattice. Then for every $a$ that conforms to $S$ and such that $\text{Gate.check}(t, t.\text{tool}, a) = \text{ALLOW}$, the call $a$ satisfies $P$.*

What the mechanised proof establishes, precisely: from gate soundness (Theorem 2) the call satisfies $t$'s constraints, and from the lattice-tightness hypothesis $t \sqsubseteq P$ those constraints entail $P$; schema conformance of $a$ is carried as a hypothesis. The step that a `PROVEN` artifact actually certifies $\Sigma \cap \neg P = \emptyset$ — i.e. that *every* schema-valid assignment satisfies $P$ — is **not** discharged inside this theorem; it is supplied by an explicit prover-soundness axiom standing for the Z3 oracle (the `proven`/citation hypotheses are recorded in the statement but the conclusion is proved from gate soundness and tightness, not from re-deriving the solver result in Lean). This is the design intent: the SMT solver, not Lean, certifies policy completeness, and the token-minting path checks the citation. The Lean development is approximately 110 lines. The full development — the shared data model plus Theorems 1–3 — is approximately 410 lines of Lean 4 and compiles under Mathlib v4.10.0 with no `sorry`s.
...
The reference implementation is an open-source library cited as `[anonymised for blind review]`. Approximately 2,500 lines of Python implement the runtime; an additional ~410 lines of Lean 4 contain the formal development. The implementation depends on Z3 (Python bindings) and the audited `cryptography` library for Ed25519. No new cryptographic primitives are introduced.
